...
const FilterButton = ({ label, value }: { label: string; value: string }) => (
    <div className="flex flex-col items-start flex-grow min-w-0 p-1.5 sm:p-2">
      <span className="text-xs font-semibold text-foreground whitespace-nowrap">{label}</span>
      <span className="text-xs sm:text-sm text-muted-foreground truncate w-full">{value}</span>
    </div>
);
...
interface ExplorerFiltersProps {
  category?: 'stays' | 'restaurants' | 'activities' | 'flights';
}

export function ExplorerFilters({ category = 'stays' }: ExplorerFiltersProps) {
  ...
  const handleSearch = () => {
    ...
    if (destination) {
      searchParams.set(category === 'activities' ? 'destination' : 'location', destination);
    }
    
    if (dateRange?.from) {
      if (category === 'activities' || category === 'restaurants') {
        searchParams.set('date', format(dateRange.from, 'PPP'));
      } else {
        searchParams.set('checkin', format(dateRange.from, 'yyyy-MM-dd'));
      }
    }
    
    if (dateRange?.to && category !== 'activities' && category !== 'restaurants') {
    ...
    if (category === 'activities') {
      // For activities, use 'guests' parameter
      searchParams.set('guests', totalGuests.toString());
    } else {
      // For stays, use separate parameters
      if (adults > 0) {
    ...
    if (selectedBudget !== 'any') {
      searchParams.set('budget', selectedBudget);
    }
    
    // Navigate to appropriate search page based on category
    ...
    if (category === 'activities') {
      ...
      // For restaurants, use guests instead of separate adults/children
      if (totalGuests > 0) {
        searchParams.set('guests', totalGuests.toString());
      }
    }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 overflow-x-hidden overflow-y-hidden">
      <div className="bg-background/90 backdrop-blur-lg rounded-full border border-border/20 shadow-lg flex items-center divide-x divide-border/30 transition-all duration-300 hover:shadow-xl focus-within:ring-2 focus-within:ring-primary/50 overflow-hidden w-full">
        
        {/* Where */}
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="ghost" className="flex-grow text-left h-16 rounded-l-full hover:bg-muted/50 px-3 sm:px-4 md:px-6 min-w-0 flex-shrink">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="ghost" className="flex-grow text-left h-16 rounded-none hover:bg-muted/50 px-3 sm:px-4 md:px-6 min-w-0 flex-shrink">
              <FilterButton
                label="When"
                value={
                  dateRange?.from
        ...
        <Popover open={isBudgetPopoverOpen} onOpenChange={setIsBudgetPopoverOpen}>
            <PopoverTrigger asChild>
                <Button variant="ghost" className="flex-grow text-left h-16 rounded-r-full hover:bg-muted/50 px-3 sm:px-4 md:px-6 min-w-0 flex-shrink">
      ...
      {/* Search Button on Separate Line */}
      <div className="flex justify-center mt-3">
        <Button 
          className="bg-primary text-primary-foreground rounded-full h-12 px-8 hover:bg-primary/90 hover:scale-105 transition-transform shadow-lg"
