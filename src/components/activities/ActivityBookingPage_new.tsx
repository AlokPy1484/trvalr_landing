...
export function ActivityBookingPage({
  ...
  useEffect(() => {
    ...
    // Initialize additional guests (participants - 1)
    const additionalCount = Math.max(0, participants - 1);
    const details = Array.from({ length: additionalCount }, () => ({
      firstName: '',
      lastName: '',
      age: '',
    }));
    setAdditionalGuests(details);
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">
                    {convertPrice(activity.priceInr)} × {participants}
                  </span>
